 ...
 * @brief Intersection-over-Union
 */
static std::vector<double> groundBoxIoU(const GroundBox& box1, const GroundBox& box2, bool require_both) {
    Polygon gp = toPolygon(box1);
    Polygon dp = toPolygon(box2);
    ...
    // union
    double o = inter_area / union_area;
    // bbox_a
    double o1 = inter_area / boost::geometry::area(gp);
    // bbox_b
    double o2 = inter_area / boost::geometry::area(dp);
    return {o1, o2, o};
}
...
static bool box3DOverlap(    const GroundBox& box1,
                             const GroundBox& box2,
                             double threshold_IoU)
{
    if (groundBoxInside(box1, box2) || groundBoxInside(box2, box1)) {
    ...
    double ymax = std::min(box1.gc_z + box1.h, box2.gc_z + box2.h);
    double ymin = std::max(box1.gc_z, box2.gc_z);

    double inter_area = in.empty() ? 0 : boost::geometry::area(in.front());
    double inter_vol = inter_area * std::max(0.0, ymax - ymin);
    

    double det_vol = box1.h * box1.l * box1.w;
    double gt_vol = box2.h * box2.l * box2.w;
