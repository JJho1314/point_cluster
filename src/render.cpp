...
void renderBox(pcl::visualization::PCLVisualizer::Ptr &viewer, std::vector<Box3D> bboxes, Color color, float opacity)
{
    int id = 0;
    for (const auto &box : bboxes)
    {
        ...
        viewer->setShapeRenderingProperties(pcl::visualization::PCL_VISUALIZER_OPACITY, opacity * 0.0, cubeFill);

        id++;
    }
}

void renderBox(pcl::visualization::PCLVisualizer::Ptr &viewer, std::vector<BoxQ> bboxes, Color color, float opacity)
{
    int id = 0;
    for (const auto &box : bboxes)
    {
