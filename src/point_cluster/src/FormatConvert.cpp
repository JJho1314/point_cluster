...
int main(int argc, char** argv) {
    ...
    {
        ROS_WARN("frame_id not set, use default value: livox_frame");
        frame_id = "livox_frame";
    }

    ros::Subscriber sub_livox_lidar = nh.subscribe<livox_ros_driver::CustomMsg>("/livox/lidar", 100, livoxLidarHandler);
    pub_ros_points = nh.advertise<sensor_msgs::PointCloud2>("/livox_full_cloud", 100);
