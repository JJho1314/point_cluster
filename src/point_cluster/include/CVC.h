...
public:
    ...
    void Process(const pcl::PointCloud<pcl::PointXYZ> &in_cloud, vector<int> &cluster_indices);
    ~CVC();
    //转换点云格式
    void CalculateAPR(const pcl::PointCloud<pcl::PointXYZ> &in_cloud, vector<APR> &APR_cloud);
    //创建Hash表
    void BuildHashTable(const vector<APR> &APR_cloud, std::unordered_map<int, Voxel> &map);
    //聚类的主函数
    vector<int> Cluster(const pcl::PointCloud<pcl::PointXYZ> &in_cloud, std::unordered_map<int, Voxel> &map, const vector<APR> &APR_cloud);
    //寻找近邻
    void FindNeighbors(const pcl::PointCloud<pcl::PointXYZ> &in_cloud, std::unordered_map<int, Voxel> &map, const int &theta_index, const int &rho_index, const int &phi_index, vector<int> &neighbors);
    //融合两个序号不同的点云
    ...
    //计算点云中心点坐标
    Pt CalculateCenter(const pcl::PointCloud<pcl::PointXYZ> &in_cloud, int self_index, std::unordered_map<int, Voxel> &map);
    //计算两个点距离是否大于阈值
